...
// ─── shared styles (defined outside — never recreated) ─────────
const inputStyle = {
  width: '100%',
  padding: '0.7rem 1rem',
...
// ─── Field — defined OUTSIDE the page component so it is never
//     recreated on re-render, which would unmount inputs mid-type.
  ...
  return (
    <button
      ...
      style={{
        ...
        boxShadow: selected ? '0 0 0 3px oklch(from var(--color-primary) l c h / 0.12)' : 'none',
      }}
    >
      <div style={{
        width: 36, height: 36, borderRadius: 'var(--radius-md)', flexShrink: 0,
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        background: selected ? 'var(--color-primary)' : 'var(--color-surface-offset)',
        color: selected ? '#fff' : 'var(--color-text-muted)',
        fontSize: '1rem', transition: 'all 160ms',
      }}>
        {iconEmoji ? iconEmoji : Icon ? <Icon size={16} /> : null}
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontWeight: 700, fontSize: 'var(--text-sm)', color: selected ? 'var(--color-primary)' : 'var(--color-text)' }}>{label}</div>
        {desc && <div style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)', marginTop: 2, lineHeight: 1.4 }}>{desc}</div>}
      </div>
      {selected && <CheckCircle2 size={16} style={{ color: 'var(--color-primary)', flexShrink: 0, marginTop: 2 }} />}
    </button>
  )
}
          ...
          <div key={step.id} style={{ display: 'flex', alignItems: 'center', flex: i < steps.length - 1 ? 1 : 'none' }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
              <div style={{
                ...
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                background: done || active ? 'var(--color-primary)' : 'var(--color-surface-offset)',
...
function FileDropzone({ files, onChange }) {
  ...
  const addFiles = (incoming) => {
    ...
    onChange(prev => {
      ...
      return [...prev, ...arr.filter(f => !existing.has(f.name + f.size))].slice(0, 5)
    })
  }
  ...
  const extColor = (name) => ({ PDF:'#e74c3c',PNG:'#0b6fb8',JPG:'#095d9c',JPEG:'#095d9c',DWG:'#e67e22',DXF:'#52b5e8' })[ext(name)] || 'var(--color-text-muted)'
        ...
        <div style={{ marginTop: '0.75rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
          {files.map(f => (
            <div key={f.name} style={{
              ...
              <span style={{ fontSize: '0.6rem', fontWeight: 800, letterSpacing: '0.04em', color: extColor(f.name), background: `${extColor(f.name)}18`, padding: '2px 6px', borderRadius: 4, flexShrink: 0 }}>{ext(f.name)}</span>
...
function PricingBadge() {
  return (
    <div style={{
      ...
      letterSpacing: '0.04em', boxShadow: '0 2px 8px rgba(11,111,184,0.35)',
    }}>
...
export default function ProfessionalDesign() {
  ...
  // Stable setter — never recreated, so child inputs keep focus
  ...
  const lookupPincode = useCallback(async (pin) => {
    ...
    try {
      ...
      if (rec?.Status === 'Success' && rec.PostOffice?.length) {
        ...
        setForm(f => ({
          ...f,
          city:  f.city  || po.Division  || po.District || '',
      ...
      setPincodeError('Lookup failed — please enter city/state manually')
    } finally {
    ...
    try {
      ...
      for (const file of files) {
        ...
        const { error: uploadErr } = await supabase.storage
          .from('order-files').upload(path, file, { contentType: file.type, upsert: false })
        if (uploadErr) throw uploadErr
        const { error: fileErr } = await supabase.from('order_files').insert({
          order_id: order.id, uploaded_by: user?.id ?? null,
          file_name: file.name, file_size_bytes: file.size,
          mime_type: file.type, storage_path: path,
          bucket: 'order-files', is_drawing: false, is_unlocked: false,
        })
    ...
    <>
      ...
      <div style={{ minHeight: '100dvh', display: 'flex', alignItems: 'center', justifyContent: 'center', paddingTop: 80, paddingInline: '1rem' }}>
        ...
        >
          ...
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'var(--text-xl)', fontWeight: 800, color: 'var(--color-text)', marginBottom: '0.75rem' }}>Request Received!</h2>
          ...
          <div style={{ padding: '0.85rem 1.1rem', background: 'var(--color-primary-highlight)', borderRadius: 'var(--radius-md)', fontSize: 'var(--text-xs)', color: 'var(--color-primary)', fontWeight: 600, marginBottom: '1.5rem' }}>
            📩 Confirmation sent to {form.email}
          </div>
          <button onClick={() => navigate('/calculator')} style={{ padding: '0.7rem 1.75rem', background: 'var(--color-primary)', color: '#fff', border: 'none', borderRadius: 'var(--radius-md)', fontWeight: 700, fontSize: 'var(--text-sm)', cursor: 'pointer' }}>Back to Calculator</button>
        </motion.div>
      </div>
    </>
  )
  ...
  return (
    <>
      <style>{`
        ...
        .pin-btn:hover:not(:disabled) { background: var(--color-primary-hover) !important; }
      ...
      <div style={{ paddingTop: 64 }}>
        {/* Hero */}
        <div style={{ background: `linear-gradient(135deg, ${BLUE_MID} 0%, ${BLUE_DARK} 100%)`, padding: 'clamp(2.5rem,5vw,4rem) 1.5rem', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'absolute', bottom: '-80px', left: '-40px', width: 220, height: 220, borderRadius: '50%', border: '30px solid rgba(255,255,255,0.04)', pointerEvents: 'none' }} />
          <div style={{ maxWidth: 700, margin: '0 auto', position: 'relative', zIndex: 1 }}>
            <div style={{ marginBottom: '0.75rem' }}><PricingBadge /></div>
            <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.75rem,4vw,2.75rem)', fontWeight: 800, color: '#fff', lineHeight: 1.15, marginBottom: '0.75rem' }}>
            ...
            <p style={{ fontSize: 'var(--text-base)', color: 'rgba(255,255,255,0.80)', maxWidth: '52ch', lineHeight: 1.7 }}>
              Tell us about your site — soil, weather zone, building type, and more. Our engineers will create a custom system design with drawings, tank sizing, and installation guidance.
            </p>
          </div>
        </div>
        ...
        <div style={{ background: 'var(--color-surface)', borderBottom: '1px solid var(--color-border)', padding: '1rem 1.5rem' }}>
          <div style={{ maxWidth: 700, margin: '0 auto', display: 'flex', flexWrap: 'wrap', gap: '1.2rem', justifyContent: 'center' }}>
            {['📐 Custom System Drawings','🔧 Component Specifications','💧 Pipe & Tank Sizing','📊 ROI Analysis Report','📞 1 Expert Consultation Call'].map(item => (
        ...
        <div style={{ maxWidth: 700, margin: '0 auto', padding: 'clamp(2rem,4vw,3rem) 1.25rem clamp(3rem,6vw,5rem)' }}>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <div style={{ background: 'var(--color-surface)', borderRadius: 'var(--radius-xl)', border: '1px solid var(--color-border)', padding: 'clamp(1.5rem,3vw,2.25rem)', boxShadow: 'var(--shadow-md)' }}>
                ...
                {step === 0 && (
                  <>
                    ...
                    {/* PIN code row with lookup button */}
                    <Field label="PIN Code">
                      <div style={{ display: 'flex', gap: '0.5rem' }}>
                        <input
                          className="pd-input"
                          style={{ ...inputStyle, flex: 1 }}
                          placeholder="e.g. 411001 — auto-fills city & state"
                          value={form.pincode}
                          maxLength={6}
                          onChange={e => {
                          ...
                          onKeyDown={e => { if (e.key === 'Enter' && form.pincode.length === 6) lookupPincode(form.pincode) }}
                        />
                        <button
                          type="button"
                          className="pin-btn"
                          onClick={() => lookupPincode(form.pincode)}
                          ...
                          style={{
                            ...
                            cursor: pincodeLoading || form.pincode.length !== 6 ? 'not-allowed' : 'pointer',
                            opacity: pincodeLoading || form.pincode.length !== 6 ? 0.5 : 1,
                        ...
                        >
                          {pincodeLoading
                            ...
                            : <Search size={13} />}
                          {pincodeLoading ? 'Looking up…' : 'Fetch'}
                        </button>
                      </div>
                      {pincodeError && (
                  ...
                  <>
                    ...
                    <div style={{ marginBottom: '1.5rem' }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(min(200px,100%),1fr))', gap: '0.5rem' }}>
                        {SOIL_TYPES.map(s => (
                          <SelectCard key={s.id} selected={form.soilType === s.id} onClick={() => set('soilType', s.id)} iconEmoji={s.icon} label={s.label} desc={s.desc} />
                        ))}
                      </div>
                    </div>
                    ...
                    <div style={{ marginBottom: '1.5rem' }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(min(200px,100%),1fr))', gap: '0.5rem' }}>
                        {WEATHER_ZONES.map(w => (
                          <SelectCard key={w.id} selected={form.weatherZone === w.id} onClick={() => set('weatherZone', w.id)} iconEmoji={w.icon} label={w.label} desc={w.desc} />
                        ))}
                      </div>
                    </div>
                    ...
                    <div style={{ marginBottom: '1rem' }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(min(200px,100%),1fr))', gap: '0.5rem' }}>
                        {BUILDING_TYPES.map(b => (
                          <SelectCard key={b.id} selected={form.buildingType === b.id} onClick={() => set('buildingType', b.id)} icon={b.icon} label={b.label} desc={b.desc} />
                        ))}
                      </div>
                    </div>
                ...
                {step === 2 && (
                  <>
                    ...
                    <div style={{ padding: '1.1rem 1.2rem', borderRadius: 'var(--radius-lg)', border: `1.5px solid ${form.visitPreferred ? 'var(--color-primary)' : 'var(--color-border)'}`, background: form.visitPreferred ? 'var(--color-primary-highlight)' : 'var(--color-surface-offset)', transition: 'all 160ms', marginBottom: '0.75rem' }}>
                      <label style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem', cursor: 'pointer' }}>
                        ...
                        <div>
                          <div style={{ fontWeight: 700, fontSize: 'var(--text-sm)', color: 'var(--color-text)', marginBottom: 2 }}>📍 I'd prefer a site visit by the engineer</div>
                          <div style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)', lineHeight: 1.5 }}>Our engineer will visit your location, assess the site in person, and provide recommendations. Travel costs are billed separately.</div>
                        </div>
                      </label>
                    </div>

                    {form.visitPreferred && (
                      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }} style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', paddingLeft: '0.25rem' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.5rem', padding: '0.75rem 1rem', marginTop: '1rem', background: BLUE_BG, borderRadius: 'var(--radius-md)', border: `1px solid ${BLUE_BORDER}` }}>
                      <span style={{ flexShrink: 0, fontSize: '0.95rem', marginTop: 1 }}>ℹ️</span>
                      <p style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)', lineHeight: 1.6, margin: 0 }}>Uploading a blueprint helps our team design more accurately. If you don't have one yet, our engineer can create a basic layout sketch during the site visit.</p>
                    </div>
                  </>
                )}
                ...
                {step === 3 && (
                  <>
                    ...
                    <div style={{ padding: '1rem 1.2rem', borderRadius: 'var(--radius-lg)', background: 'var(--color-surface-offset)', border: '1px solid var(--color-border)', marginTop: '0.5rem' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.6rem', padding: '0.85rem 1rem', marginTop: '1rem', borderRadius: 'var(--radius-md)', background: BLUE_BG, border: `1px solid ${BLUE_BORDER}` }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '1.25rem', gap: '0.75rem' }}>
            <button type="button" onClick={step === 0 ? () => navigate(-1) : prev} className="pd-prev"
              style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', padding: '0.7rem 1.25rem', border: '1.5px solid var(--color-border)', borderRadius: 'var(--radius-md)', background: 'var(--color-surface)', color: 'var(--color-text-muted)', fontSize: 'var(--text-sm)', fontWeight: 600, cursor: 'pointer', transition: 'background 160ms' }}>
              <ArrowLeft size={15} />{step === 0 ? 'Back' : 'Previous'}
            </button>

            {step < STEPS.length - 1 ? (
              <button type="button" onClick={next} disabled={!canProceed} className="pd-next"
                style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', padding: '0.7rem 1.75rem', background: 'var(--color-primary)', color: '#fff', border: 'none', borderRadius: 'var(--radius-md)', fontSize: 'var(--text-sm)', fontWeight: 700, cursor: 'pointer', transition: 'background 160ms' }}>
                Continue <ArrowRight size={15} />
              </button>
            ) : (
              <button type="button" onClick={submit} disabled={!canProceed || loading} className="pd-next"
                style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', padding: '0.7rem 1.75rem', background: 'var(--color-primary)', color: '#fff', border: 'none', borderRadius: 'var(--radius-md)', fontSize: 'var(--text-sm)', fontWeight: 700, cursor: 'pointer', transition: 'background 160ms', minWidth: 180, justifyContent: 'center' }}>
                {loading ? <><Loader2 size={15} style={{ animation: 'spin 0.9s linear infinite' }} /> Submitting…</> : <><Star size={14} /> Submit Request</>}
              </button>
            )}
          </div>
