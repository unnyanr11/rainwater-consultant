        ...
        <main className="admin-main">
          ...
          <div style={{ display: 'flex', gap: '0.65rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
            <div style={{ position: 'relative', flex: '1 1 220px', minWidth: 0 }}>
              <Search size={14} style={{ position: 'absolute', left: '0.75rem', top: '50%', transform: 'translateY(-50%)', color: 'var(--color-text-faint)', pointerEvents: 'none' }} />
              <input
                type="text"
                placeholder="Search name, email, phone, city…"
                value={search}
                onChange={e => setSearch(e.target.value)}
                style={{ width: '100%', padding: '0.55rem 0.75rem 0.55rem 2.25rem', border: '1.5px solid var(--color-border)', borderRadius: 'var(--radius-md)', background: 'var(--color-surface)', color: 'var(--color-text)', fontSize: 'var(--text-sm)', outline: 'none' }}
              />
            </div>

            <div style={{ position: 'relative' }}>
              <Filter size={13} style={{ position: 'absolute', left: '0.7rem', top: '50%', transform: 'translateY(-50%)', color: 'var(--color-text-faint)', pointerEvents: 'none' }} />
              <select
                value={statusFilter}
                onChange={e => setStatusFilter(e.target.value)}
                style={{ padding: '0.55rem 0.75rem 0.55rem 2.1rem', border: '1.5px solid var(--color-border)', borderRadius: 'var(--radius-md)', background: 'var(--color-surface)', color: 'var(--color-text)', fontSize: 'var(--text-sm)', outline: 'none', cursor: 'pointer' }}
              >
          ...
          {error && (
            <div style={{ padding: '1rem 1.25rem', borderRadius: 'var(--radius-md)', background: 'rgba(161,44,123,0.08)', border: '1px solid rgba(161,44,123,0.25)', color: 'var(--color-error)', fontSize: 'var(--text-sm)', marginBottom: '1rem' }}>
          ...
          {!loading && !error && filtered.length === 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', padding: 'var(--space-16) var(--space-8)', color: 'var(--color-text-muted)' }}>
                ...
                <thead>
                  <tr style={{ borderBottom: '2px solid var(--color-border)' }}>
                    {['Contact', 'Location', 'Building', 'Roof Area', 'Visit', 'Status', 'Submitted', ''].map(h => (
                      <th key={h} style={{ padding: '0.5rem 0.85rem', textAlign: 'left', fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: '0.07em', whiteSpace: 'nowrap' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {filtered.map((lead) => (
                    <tr
                      ...
                      style={{ borderBottom: '1px solid var(--color-border)', cursor: 'pointer', transition: 'background 140ms' }}
                      onMouseEnter={e => e.currentTarget.style.background = 'var(--color-surface-offset)'}
                      onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
                    >
                      <td style={{ padding: '0.75rem 0.85rem' }}>
                        <div style={{ fontWeight: 700, fontSize: 'var(--text-sm)', color: 'var(--color-text)' }}>{lead.contact_name || '—'}</div>
                        <div style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)', marginTop: 2 }}>{lead.contact_phone || lead.contact_email || '—'}</div>
                      </td>
                      <td style={{ padding: '0.75rem 0.85rem' }}>
                        <div style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text)' }}>{lead.city || '—'}</div>
                        <div style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)', marginTop: 2 }}>{lead.state}{lead.pincode ? ` · ${lead.pincode}` : ''}</div>
                      </td>
                      <td style={{ padding: '0.75rem 0.85rem' }}>
                        <span style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text)' }}>{BUILDING_LABELS[lead.building_type] || lead.building_type || '—'}</span>
                        {lead.storeys ? (<div style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)', marginTop: 2 }}>{lead.storeys} {lead.storeys === 1 ? 'storey' : 'storeys'}</div>) : null}
                      </td>
                      <td style={{ padding: '0.75rem 0.85rem' }}>
                      ...
                      <td style={{ padding: '0.75rem 0.85rem' }}>
                        {lead.visit_preferred ? (
                          <span style={{ fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--color-success)', background: 'rgba(67,122,34,0.1)', padding: '0.2rem 0.6rem', borderRadius: 99 }}>Requested</span>
                        ) : (
                      ...
                      <td style={{ padding: '0.75rem 0.85rem' }}><StatusBadge status={lead.status} /></td>
                      <td style={{ padding: '0.75rem 0.85rem' }}>
