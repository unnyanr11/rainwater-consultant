  ...
  return (
    <div style={{ maxWidth: 720 }}>
      ...
      <section style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-lg)', padding: '1.25rem', marginBottom: '1.25rem' }}>
        ...
        <Row label="Quote"            value={order.quoted_amount ? `₹${Number(order.quoted_amount).toLocaleString('en-IN')}` : null} />
            ...
            {files.map(f => (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  ...
                  <span style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text)' }}>{f.file_name || f.file_path?.split('/').pop()}</span>
                </div>
                <button
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {log.map((entry, i) => (
              <div key={entry.id} style={{ display: 'flex', gap: '0.75rem', alignItems: 'flex-start' }}>
                <div style={{ width: 8, height: 8, borderRadius: '50%', background: i === log.length - 1 ? 'var(--color-primary)' : 'var(--color-border)', flexShrink: 0, marginTop: 5 }} />
                <div>
